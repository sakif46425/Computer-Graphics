...
/*
if (trafficLight == GREEN) {  // Cars move normally but reversed
    _carOne -= 3.0;
    if (_carOne < -900) _carOne = 250;

    _carTwo -= 5.0;
    if (_carTwo < -1050) _carTwo += 2000;

    _carThree += 2.0;
    if (_carThree > 640) _carThree = -640;

    _carFour += 6.0;
    if (_carFour > 940) _carFour = -740;
}
else if (trafficLight == YELLOW) {  // Cars move slower but reversed
    _carOne -= 1.5;
    if (_carOne < -900) _carOne = 250;
...
// Boats always move but reversed
_boatOne -= 0.80;
if (_boatOne < -1300) {
    _boatOne = 1200;
}
...
if (_cloudThree < -1300) {
   ...
   */
   ...
   /* _plane -= 0.70;
if (_plane < -600) {  // Adjust the reset value based on your coordinate system
    _plane += 1300;
}*/
